...
describe('Internal Linking Utilities', () => {
  ...
  describe('getInternalLinksForPage', () => {
    ...
    it('returns about page linking strategy', () => {
      ...
      const aboutLinks = links?.relatedPages.filter(link => link.href.startsWith('/about/'));
    ...
    it('returns about subpage linking strategy', () => {
      ...
      const relatedAboutLinks = links?.relatedPages.filter(
        link => link.href.startsWith('/about/') && link.href !== '/about/our-story'
      );
    ...
    it('returns services page linking strategy', () => {
      ...
      const serviceLinks = links?.relatedPages.filter(link => link.href.startsWith('/services/'));
    ...
    it('returns service subpage linking strategy', () => {
      ...
      const otherServiceLinks = links?.relatedPages.filter(
        link =>
          link.href.startsWith('/services/') && link.href !== '/services/video-production-morocco'
      );
    ...
    it('returns portfolio page linking strategy', () => {
      ...
      const hasServicesLink = links?.relatedPages.some(link => link.href === '/services');
    ...
    it('returns contact page linking strategy', () => {
      ...
      const hasServicesLink = links?.relatedPages.some(link => link.href === '/services');
      const hasPortfolioLink = links?.relatedPages.some(link => link.href === '/portfolio');
      expect(hasServicesLink).toBe(true);
    ...
    it('includes call-to-action links for appropriate pages', () => {
      ...
      const aboutHasContact = aboutLinks?.callToActionLinks.some(link => link.href === '/contact');
    ...
    it('assigns correct priority levels', () => {
      ...
      const highPriorityLinks = links?.relatedPages.filter(link => link.priority === 'high');
      expect(highPriorityLinks?.length).toBeGreaterThan(0);

      // Should have medium priority links
      const mediumPriorityLinks = links?.relatedPages.filter(link => link.priority === 'medium');
    ...
    it('includes contextual information', () => {
      ...
      const contextualLinks = links?.contextualLinks.filter(link => link.context);
  ...
  describe('getRelatedServicePages', () => {
    it('returns related services for video production', () => {
      ...
      const includesCurrentService = relatedServices.some(
        service => service.href === '/services/video-production-morocco'
      );
      expect(includesCurrentService).toBe(false);

      // Should include other services
      const hasOtherServices = relatedServices.some(
        service =>
    ...
    it('returns related services for 3D visualization', () => {
      ...
      const hasPhotography = relatedServices.some(service => service.href.includes('photography'));
  ...
  describe('Link Quality and Consistency', () => {
    it('ensures all links have required properties', () => {
      ...
      testPages.forEach(page => {
        ...
        if (links) {
          [...links.relatedPages, ...links.contextualLinks, ...links.callToActionLinks].forEach(
            link => {
    ...
    it('ensures no circular references in related links', () => {
      ...
      if (links) {
        const hasCircularReference = links.relatedPages.some(
          link => link.href === '/about/our-story'
        );
