...
global.CSS = {
  supports: jest.fn((property: string, value: string) => {
    ...
    return supportedFeatures.some(feature => 
      feature.includes(property) && feature.includes(value)
    );
  }),
} as any;
...
describe('BrowserTestUtils', () => {
  describe('Modern Browser Detection', () => {
    it('should identify modern browsers', () => {
      const modernUA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36';
      expect(BrowserTestUtils.isModernBrowser(modernUA)).toBe(true);
    });

    it('should identify old browsers', () => {
      const oldUA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.77 Safari/537.36';
