...
import { getInternalLinksForPage, getRelatedServicePages, InternalLink } from '@/lib/utils/internal-linking';
  ...
  const renderLink = (link: InternalLink, index: number) => (
    <ScrollReveal key={link.href} delay={index * 0.1}>
      <MagneticField strength={0.2} distance={100}>
        <Link
          href={link.href}
          className="group block rounded-lg border border-bw-border-subtle bg-bw-bg-secondary p-4 transition-all duration-300 hover:border-bw-accent-gold hover:bg-bw-bg-primary hover:shadow-lg"
        >
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <h4 className="font-medium text-bw-text-primary group-hover:text-bw-accent-gold transition-colors duration-300">
                {link.text}
              </h4>
              {link.description && (
        ...
        {relatedServices.map((service, index) => (
          <ScrollReveal key={service.href} delay={index * 0.1}>
            <MagneticField strength={0.2} distance={100}>
              <Link
                href={service.href}
                className="group block rounded-lg border border-bw-border-subtle bg-bw-bg-secondary p-6 transition-all duration-300 hover:border-bw-accent-gold hover:bg-bw-bg-primary hover:shadow-lg"
              >
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-bw-text-secondary">
                      {service.description}
                    </p>
                  </div>
...
export function ContextualNavigation({ 
  ...
  return (
    <nav className={`inline-flex items-center space-x-4 ${className}`} aria-label="Contextual navigation">
